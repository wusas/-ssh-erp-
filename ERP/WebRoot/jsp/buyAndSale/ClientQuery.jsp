...
<body>
	<center>
		<form action="" method="post" name="f" class="navbar-form navbar-left">
			客户姓名：<input type="text" name="clientName" id="clientName" />
			客户等级： <select name="clientGrade">
				<option value="-99" selected="selected">所有</option>
				<option value="1">高</option>
				<option value="2">中</option>
				<option value="3">低</option>
			</select> 
			是否删除： <select name="clientDel">
...
function query(a) {
	...
	var clientDel = f.clientDel.value;
	var clientName = f.clientName.value;
	var clientGrade = f.clientGrade.value;
	if(a==1){
		pagecur = 1;
	}else if(a==2){//查询上一页
		pagecur = pagecur-1;
		...
		$(document).ready(function() {
			$.post("<%=path%>/client_findBySome.action", {
				"clientDel":clientDel,
...
*/
function goIns(){
	window.showModalDialog("<%=path%>/jsp/buyAndSale/ClientInsert.jsp",window,"dialogHeight:300px;dialogWidth:400px;")
}
/*
	修改
*/
function goUpd(id_key){
	window.showModalDialog("<%=path%>/client_findById.action?clientId="+id_key,window,"dialogHeight:300px;dialogWidth:400px;")
	}
	/*
		删除
	*/
	...
	function goBack(id_key) {
		if (confirm("确认还原？")) {
			$(document).ready(function() {
				$.post("<%=path%>/client_doBack.action", {
					"clientId" : id_key
				}, function(data) {
					if (data.indexOf("true") != -1) {
						alert("还原成功");
